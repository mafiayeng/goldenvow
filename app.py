...
# Email (optional – only if EMAIL_PASS is set)
...
with app.app_context():
    ...
    db.session.commit()
    # Simple column migration for SQLite (ignore if fails)
    try:
...
# ---------- BASE HTML (WITH LOGO, WHATSAPP FLOAT, GLASS UI) ----------
BASE_HTML = """
<!DOCTYPE html>
<html>
...
def edit_event(token):
    ...
    if request.method == 'POST':
        ...
        return redirect(url_for('dashboard'))
    return render_template_string(CREATE_EVENT_HTML)  # reuse
...
def manage_admins():
    ...
    admins = Admin.query.all()
    return render_template_string("""
    <h2>Manage Admins</h2><ul>{% for a in admins %}<li>{{ a.username }} - {{ 'Super' if a.is_super_admin else 'Admin' }} - {% if a.is_active %}Active{% else %}Disabled{% endif %}</li>{% endfor %}</ul>
    """, admins=admins)
...
def settings():
    ...
    if request.method == 'POST':
        ...
        if setting:
            setting.value = 'True' if maintenance else 'False'
        db.session.commit()
        flash('Settings updated.', 'success')
        return redirect(url_for('settings'))
    setting = Setting.query.filter_by(key='maintenance_mode').first()
    return render_template_string("""
    <h2>Settings</h2><form method="POST"><input type="checkbox" name="maintenance_mode" {% if setting.value == 'True' %}checked{% endif %}> Maintenance mode<button type="submit">Save</button></form>
    """, setting=setting)

# =============================================================================
# GOLDENVOW – FULL SELF-CONTAINED APPLICATION (No Template Inheritance)
# =============================================================================
...
# ---------- HELPERS ----------
def is_admin_logged_in(): return session.get('admin_id') is not None
def is_contributor_logged_in(): return session.get('contributor_id') is not None
def get_admin():
    if not is_admin_logged_in(): return None
    return Admin.query.get(session['admin_id'])
def get_contributor():
    if not is_contributor_logged_in(): return None
    return Contributor.query.get(session['contributor_id'])
def is_super_admin():
    admin = get_admin()
    return admin and admin.is_super_admin
def generate_unique_token(): return str(uuid.uuid4())[:12]
def generate_pin(): return f"{random.randint(1000, 9999)}"
def generate_referral_code(): return f"GV-{''.join(random.choices(string.ascii_uppercase + string.digits, k=6))}"
def hash_password(plain): return bcrypt.hashpw(plain.encode('utf-8'), bcrypt.gensalt()).decode('utf-8')
def check_password(plain, hashed): return bcrypt.checkpw(plain.encode('utf-8'), hashed.encode('utf-8'))
...
def get_unread_notifications(admin_id): return Notification.query.filter_by(admin_id=admin_id, is_read=False).count()
def get_fee_percentage(admin_id):
    admin = Admin.query.get(admin_id)
    if not admin: return SERVICE_FEE_PERCENTAGE
...
def get_admin_total_fees(admin_id):
    ...
    total = 0
    for e in events: total += get_event_total_fee(e.id)
    return total
def is_fee_overdue(event):
    if not event.first_contribution_date: return False
    if event.fee_paid: return False
    total_fee = get_event_total_fee(event.id)
    if total_fee < 50.0: return False
    due_date = event.first_contribution_date + timedelta(days=3)
    if datetime.utcnow() > due_date:
        grace_end = due_date + timedelta(hours=1)
        if datetime.utcnow() > grace_end: return True
    return False
def get_page_lock_status(event, contributor_token=None):
    if event.disabled: return True
    if contributor_token: return False
    if not event.first_contribution_date: return False
    if event.fee_paid: return False
    if is_fee_overdue(event): return True
    return False
...
<link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
<style>body{background:#0b1120;color:#e2e8f0;font-family:'Inter',Arial,sans-serif;min-height:100vh;display:flex;align-items:center;justify-content:center;}.glass-card{background:rgba(255,255,255,0.06);backdrop-filter:blur(12px);border:1px solid rgba(255,255,255,0.08);border-radius:24px;padding:40px;box-shadow:0 20px 40px rgba(0,0,0,0.4);max-width:420px;width:100%;}.golden-text{color:#D4AF37}.btn-gold{background:linear-gradient(135deg,#D4AF37,#f5d06b);border:none;color:#0b1120;font-weight:600;border-radius:50px;padding:10px 24px;width:100%}input{background:#1a2332 !important;color:#fff !important;border:1px solid rgba(255,255,255,0.2) !important;border-radius:12px !important;padding:12px 16px !important;width:100%;margin-bottom:12px}a{color:#D4AF37}</style>
</head><body>
<div class="glass-card"><h3 class="text-center golden-text">✦ Admin Login</h3><hr>
<form method="POST" autocomplete="on"><input type="text" name="username" placeholder="Username" autocomplete="username" required><input type="password" name="password" placeholder="Password" autocomplete="current-password" required><button type="submit" class="btn-gold">Login</button></form>
<div class="mt-3 text-center"><a href="/forgot-password">Forgot password?</a><br><small>Don't have an account? <a href="/register">Register</a></small><hr><small>Contributor? <a href="/contributor/login">Login here</a></small></div></div>
</body></html>"""

REGISTER_HTML = """<!DOCTYPE html>
<html><head><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Register</title>
<link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
<style>body{background:#0b1120;color:#e2e8f0;font-family:'Inter',Arial,sans-serif;min-height:100vh;display:flex;align-items:center;justify-content:center;}.glass-card{background:rgba(255,255,255,0.06);backdrop-filter:blur(12px);border:1px solid rgba(255,255,255,0.08);border-radius:24px;padding:40px;box-shadow:0 20px 40px rgba(0,0,0,0.4);max-width:460px;width:100%;}.golden-text{color:#D4AF37}.btn-gold{background:linear-gradient(135deg,#D4AF37,#f5d06b);border:none;color:#0b1120;font-weight:600;border-radius:50px;padding:10px 24px;width:100%}input{background:#1a2332 !important;color:#fff !important;border:1px solid rgba(255,255,255,0.2) !important;border-radius:12px !important;padding:12px 16px !important;width:100%;margin-bottom:12px}a{color:#D4AF37}</style>
</head><body>
...
<style>body{background:#0b1120;color:#e2e8f0;font-family:'Inter',Arial,sans-serif;padding:20px}.golden-text{color:#D4AF37}.glass-card{background:rgba(255,255,255,0.06);backdrop-filter:blur(12px);border:1px solid rgba(255,255,255,0.08);border-radius:24px;padding:20px;box-shadow:0 20px 40px rgba(0,0,0,0.4)}.btn-gold{background:linear-gradient(135deg,#D4AF37,#f5d06b);border:none;color:#0b1120;font-weight:600;border-radius:50px;padding:10px 20px}.btn-outline-gold{border:2px solid #D4AF37;color:#D4AF37;border-radius:50px;background:transparent}.btn-outline-gold:hover{background:#D4AF37;color:#0b1120}.progress-bar-gold{background:linear-gradient(90deg,#D4AF37,#f5d06b)}</style>
</head><body>
...
def dashboard():
    ...
    return render_template_string(DASHBOARD_HTML, admin=admin, events=events, total_raised=total_raised, pending_contributions=pending_count)
...
<link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.10.0/font/bootstrap-icons.css">
<style>body{background:#0b1120;color:#e2e8f0;padding:20px}.golden-text{color:#D4AF37}.glass-card{background:rgba(255,255,255,0.06);backdrop-filter:blur(12px);border:1px solid rgba(255,255,255,0.08);border-radius:24px;padding:20px;box-shadow:0 20px 40px rgba(0,0,0,0.4)}.btn-gold{background:linear-gradient(135deg,#D4AF37,#f5d06b);border:none;color:#0b1120;border-radius:50px;padding:10px 20px}</style>
</head><body>
...
<link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
<style>body{background:#0b1120;color:#e2e8f0;font-family:'Inter',Arial,sans-serif;min-height:100vh;display:flex;align-items:center;justify-content:center;}.glass-card{background:rgba(255,255,255,0.06);backdrop-filter:blur(12px);border:1px solid rgba(255,255,255,0.08);border-radius:24px;padding:40px;max-width:420px;width:100%;}.golden-text{color:#D4AF37}.btn-gold{background:linear-gradient(135deg,#D4AF37,#f5d06b);border:none;color:#0b1120;border-radius:50px;padding:10px 24px;width:100%}input{background:#1a2332 !important;color:#fff !important;border:1px solid rgba(255,255,255,0.2) !important;border-radius:12px !important;padding:12px 16px !important;width:100%;margin-bottom:12px}</style>
</head><body><div class="glass-card"><h3 class="golden-text text-center">Contributor Login</h3>
...
<div class="mt-3 text-center"><a href="/contributor/register?event_token={{ request.args.get('event_token', '') }}">Create account</a></div></div></body></html>""")
...
<link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
<style>body{background:#0b1120;color:#e2e8f0;font-family:'Inter',Arial,sans-serif;min-height:100vh;display:flex;align-items:center;justify-content:center;}.glass-card{background:rgba(255,255,255,0.06);backdrop-filter:blur(12px);border:1px solid rgba(255,255,255,0.08);border-radius:24px;padding:40px;max-width:460px;width:100%;}.golden-text{color:#D4AF37}.btn-gold{background:linear-gradient(135deg,#D4AF37,#f5d06b);border:none;color:#0b1120;border-radius:50px;padding:10px 24px;width:100%}input{background:#1a2332 !important;color:#fff !important;border:1px solid rgba(255,255,255,0.2) !important;border-radius:12px !important;padding:12px 16px !important;width:100%;margin-bottom:12px}</style>
</head><body><div class="glass-card"><h3 class="golden-text text-center">Contributor Register</h3>
...
<div class="mt-3 text-center"><a href="/contributor/login?event_token={{ request.args.get('event_token', '') }}">Already have an account?</a></div></div></body></html>""")
...
<link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
<style>body{background:#0b1120;color:#e2e8f0;padding:20px}.golden-text{color:#D4AF37}.glass-card{background:rgba(255,255,255,0.06);backdrop-filter:blur(12px);border:1px solid rgba(255,255,255,0.08);border-radius:24px;padding:20px}</style>
</head><body><div class="container"><h2 class="golden-text">Contributor Dashboard</h2><p>Name: {{ contrib.name }}</p><ul>{% for c in contributions %}<li>{{ c.event.title }} - {{ c.status }}</li>{% endfor %}</ul></div></body></html>""", contrib=contrib, contributions=contributions)
...
<link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
<style>body{background:#0b1120;color:#e2e8f0;padding:20px}.golden-text{color:#D4AF37}.glass-card{background:rgba(255,255,255,0.06);backdrop-filter:blur(12px);border:1px solid rgba(255,255,255,0.08);border-radius:24px;padding:20px}.btn-gold{background:linear-gradient(135deg,#D4AF37,#f5d06b);border:none;color:#0b1120;border-radius:50px;padding:10px 20px}input,textarea,select{background:#1a2332 !important;color:#fff !important;border:1px solid rgba(255,255,255,0.2) !important;border-radius:12px !important;padding:10px 16px !important;width:100%;margin-bottom:10px}</style>
</head><body><div class="container"><h2 class="golden-text">Create Event</h2>
...
<link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.10.0/font/bootstrap-icons.css">
<style>body{background:#0b1120;color:#e2e8f0;padding:20px}.golden-text{color:#D4AF37}.glass-card{background:rgba(255,255,255,0.06);backdrop-filter:blur(12px);border:1px solid rgba(255,255,255,0.08);border-radius:24px;padding:20px;box-shadow:0 20px 40px rgba(0,0,0,0.4)}.btn-gold{background:linear-gradient(135deg,#D4AF37,#f5d06b);border:none;color:#0b1120;border-radius:50px;padding:10px 20px}.progress-bar-gold{background:linear-gradient(90deg,#D4AF37,#f5d06b)}</style>
</head><body><div class="container">
<div class="glass-card text-center"><div>{{ generate_event_logo(event, 100)|safe }}</div><h1 class="golden-text">{{ event.title }}</h1><p>{{ event.description }}</p>
...
</div></body></html>""", event=event, total_raised=total_raised, chat_messages=chat_messages, testimonials=testimonials, daily_note=daily_note, contributor=contributor)
...
<link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
<style>body{background:#0b1120;color:#e2e8f0;font-family:'Inter',Arial,sans-serif;min-height:100vh;display:flex;align-items:center;justify-content:center;}.glass-card{background:rgba(255,255,255,0.06);backdrop-filter:blur(12px);border:1px solid rgba(255,255,255,0.08);border-radius:24px;padding:40px;max-width:460px;width:100%;}.golden-text{color:#D4AF37}.btn-gold{background:linear-gradient(135deg,#D4AF37,#f5d06b);border:none;color:#0b1120;border-radius:50px;padding:10px 24px;width:100%}input,textarea{background:#1a2332 !important;color:#fff !important;border:1px solid rgba(255,255,255,0.2) !important;border-radius:12px !important;padding:12px 16px !important;width:100%;margin-bottom:12px}</style>
</head><body><div class="glass-card"><h3 class="golden-text text-center">📩 Contact Us</h3>
...
<div class="mt-3 text-center"><p>WhatsApp: {{ support_whatsapp }}</p><p>Email: <a href="mailto:{{ support_email }}" style="color:#D4AF37;">{{ support_email }}</a></p></div></div></body></html>""")
...
<link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
<style>body{background:#0b1120;color:#e2e8f0;padding:20px}.golden-text{color:#D4AF37}.glass-card{background:rgba(255,255,255,0.06);backdrop-filter:blur(12px);border:1px solid rgba(255,255,255,0.08);border-radius:24px;padding:20px;max-width:500px;margin:auto}.btn-gold{background:linear-gradient(135deg,#D4AF37,#f5d06b);border:none;color:#0b1120;border-radius:50px;padding:10px 20px;width:100%}input{background:#1a2332 !important;color:#fff !important;border:1px solid rgba(255,255,255,0.2) !important;border-radius:12px !important;padding:10px 16px !important;width:100%;margin-bottom:10px}</style>
</head><body><div class="glass-card"><h3 class="golden-text">My Profile</h3>
...
<button type="submit" class="btn-gold">Update</button></form></div></body></html>""", admin=admin)
...
def notifications():
    ...
    return render_template_string("""<h1>Notifications</h1><ul>{% for n in notifs %}<li>{{ n.message }} - {{ n.created_at }}</li>{% endfor %}</ul>""", notifs=notifs)
...
<link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
<style>body{background:#0b1120;color:#e2e8f0;padding:20px}.golden-text{color:#D4AF37}.glass-card{background:rgba(255,255,255,0.06);backdrop-filter:blur(12px);border:1px solid rgba(255,255,255,0.08);border-radius:24px;padding:20px;max-width:500px;margin:auto}.btn-gold{background:linear-gradient(135deg,#D4AF37,#f5d06b);border:none;color:#0b1120;border-radius:50px;padding:10px 20px}</style>
</head><body><div class="glass-card"><h2 class="golden-text">Settings</h2>
<form method="POST"><div class="form-check mb-3"><input type="checkbox" name="maintenance_mode" id="maintenance" {% if maintenance_mode %}checked{% endif %} class="form-check-input"><label for="maintenance">Enable Maintenance Mode</label></div><button type="submit" class="btn-gold">Save</button></form></div></body></html>""", maintenance_mode=maintenance_mode)
...
def withdrawals():
    ...
    return render_template_string("""<h1>Withdrawals</h1><ul>{% for w in wd_list %}<li>{{ w.amount }} - {{ w.status }}</li>{% endfor %}</ul>""", withdrawals=wd_list)
...
<link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
<style>body{background:#0b1120;color:#e2e8f0;padding:20px}.golden-text{color:#D4AF37}.glass-card{background:rgba(255,255,255,0.06);backdrop-filter:blur(12px);border:1px solid rgba(255,255,255,0.08);border-radius:24px;padding:20px}</style>
</head><body><div class="container"><div class="glass-card"><h1 class="golden-text">Help & FAQ</h1>
...
def manage_admins():
    if not is_admin_logged_in() or not get_admin().is_super_admin:
        ...
        return redirect(url_for('login'))
    admins = Admin.query.all()
    return render_template_string("""<h1>Manage Admins</h1><ul>{% for a in admins %}<li>{{ a.username }} - {{ 'Super' if a.is_super_admin else 'Admin' }} - {% if a.is_active %}Active{% else %}Disabled{% endif %}</li>{% endfor %}</ul>""", admins=admins)

...
from flask import Flask, render_template_string, request, redirect, url_for, session, flash, jsonify, send_file, abort
...
class Event(db.Model):
    ...
    category = db.Column(db.String(50), nullable=False)  # dowry, burial, medical, education, harambee
...
class ChatMessage(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    sender_type = db.Column(db.String(20), nullable=False)  # admin, contributor, super
...
class Notification(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    user_type = db.Column(db.String(20), nullable=False)  # admin, contributor
...
def check_event_lock(event):
    ...
    # Lock if unpaid fees >= 50 and overdue by 3 days after first contribution
    if total_unpaid >= 50:
...
<head>
    ...
    <style>
        :root { --primary: #d4af37; --secondary: #1a1a2e; }
        ...
        .card-shadow { box-shadow: 0 4px 20px rgba(0,0,0,0.08); border: none; border-radius: 15px; }
        ...
        .admin-sidebar { background: var(--secondary); min-height: 100vh; padding: 20px; }
        ...
        @media (max-width: 768px) { .admin-sidebar { min-height: auto; } }
    </style>
    ...
    <nav class="navbar navbar-expand-lg navbar-gold">
        <div class="container">
            <a class="navbar-brand" href="{{ url_for('landing') }}">
                <i class="bi bi-award"></i> GoldenVow
            </a>
...
{% extends 'base.html' %}
{% block content %}
<div class="hero-section text-center py-5" style="background: linear-gradient(135deg, #1a1a2e, #16213e); color: #fff;">
    <div class="container">
...
def create_super_admin():
    ...
    secret = os.environ.get('SUPER_ADMIN_SECRET', 'super.mfy')
    admin = Admin(
        username='super',
...
def login():
    if request.method == 'POST':
        ...
        if contributor:
            # Simple check: if password matches (store password in future, for now use phone as pass)
            if password == contributor.phone:  # Simplified; you should add password field later
                session.permanent = True
...
<div class="row">
    <div class="col-md-3 admin-sidebar">
        <h5 class="text-gold">Welcome, {{ session.username }}</h5>
...
def generate_receipt(payment_id):
    payment = Payment.query.get_or_404(payment_id)
    # Allow admin anytime, contributor after 7 days
    if session.get('user_type') in ['admin', 'super']:
        pass
...
def event_landing(event_token):
    ...
    contributors = Contributor.query.filter_by(event_id=event.id).all()
    payments = Payment.query.filter_by(event_id=event.id, status='approved').all()
        ...
        <div class="card card-shadow mt-3">
            <div class="card-body">
                ...
                <form action="{{ url_for('contributor_register', event_token=event.event_token) }}" method="post">
                    ...
                    <div class="mb-2"><input type="text" name="phone" placeholder="Phone" class="form-control"></div>
...
{% endblock %}
""", event=event, contributors=contributors, payments=payments, testimonials=testimonials)
...
def contributor_register(event_token):
    ...
    db.session.commit()
    # Auto create a payment record (pending)
    payment = Payment(
...
def upload_proof(event_token, contributor_id):
    ...
    if request.method == 'POST':
        # Simulate file upload - store as text for simplicity
        proof_text = request.form.get('proof_text', 'Payment sent via M-PESA')
        # Auto-verify sender name (if matched)
        ...
        if payment:
            ...
            payment.proof_image = proof_text  # store text as proof
            # Auto-approve if sender name matches
...
<div class="row g-3 mb-4">
    ...
    <div class="col-md-3"><div class="card card-shadow p-3"><h5>Total Fees Collected</h5><h2>KES {{ "%.2f"|format(total_fees) }}</h2></div></div>
</div>
<div class="row">
    <div class="col-md-6">
        <h5>Manage Admins</h5>
...
<a href="{{ url_for('toggle_maintenance') }}" class="btn btn-outline-secondary mt-3">
...
def request_withdrawal():
    if request.method == 'POST':
        ...
        admin = Admin.query.get(session['user_id'])
        # Check if fees collected enough
        total_fees = db.session.query(db.func.sum(Payment.fee_deducted)).scalar() or 0
...
def send_reminders():
    with app.app_context():
        # Send reminder for pending payments older than 2 days

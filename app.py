...
# ---------- BASE HTML (with back button, auto-save, clickable email) ----------
BASE_HTML = """
<!DOCTYPE html>
<html>
...
<!-- Back button (visible on all pages except the landing intro) -->
...
<footer class="footer mt-5 py-3"><div class="container text-center"><p class="mb-1 golden-text">✦ GoldenVow – Bringing Communities Together</p><small class="text-muted-light">WhatsApp: {{ support_whatsapp }} &nbsp;|&nbsp; Email: <a href="mailto:{{ support_email }}" style="color: #D4AF37;">{{ support_email }}</a></small></div></footer>
...
<section class="hero">
    <div class="glass-card p-5" style="max-width: 800px; margin: 0 auto;">
        ...
        <div class="row mt-4">
            <div class="col-md-4"><i class="bi bi-calendar-event fs-1 golden-text"></i><h5>Create Events</h5><p class="small text-muted-light">Set up fundraising events in minutes.</p></div>
            <div class="col-md-4"><i class="bi bi-people fs-1 golden-text"></i><h5>Manage Contributors</h5><p class="small text-muted-light">Track pledges, uploads, and approvals.</p></div>
            <div class="col-md-4"><i class="bi bi-graph-up fs-1 golden-text"></i><h5>Earn Fees</h5><p class="small text-muted-light">2% platform fee on every contribution.</p></div>
        </div>
    </div>
</section>
<script>
...
{% extends "base.html" %}
{% block content %}
<div class="row justify-content-center"><div class="col-md-5 col-lg-4"><div class="glass-card p-4"><h3 class="text-center golden-text">✦ Admin Login</h3><hr><form method="POST" autocomplete="on"><div class="mb-3"><label class="form-label">Username</label><input type="text" name="username" class="form-control" placeholder="Enter username" autocomplete="username" required></div><div class="mb-3"><label class="form-label">Password</label><input type="password" name="password" class="form-control" placeholder="Enter password" autocomplete="current-password" required></div><button type="submit" class="btn btn-gold w-100">Login</button></form><div class="mt-3 text-center"><a href="{{ url_for('forgot_password') }}" class="text-muted-light">Forgot password?</a><br><small class="text-muted-light">Don't have an account? <a href="{{ url_for('register') }}" class="golden-text">Register</a></small><hr><small class="text-muted-light">Contributor? <a href="{{ url_for('contributor_login') }}" class="golden-text">Login here</a></small></div></div></div></div>
{% endblock %}
"""

CONTRIBUTOR_LOGIN_HTML = """
{% extends "base.html" %}
{% block content %}
<div class="row justify-content-center"><div class="col-md-5"><div class="glass-card p-4"><h3 class="golden-text text-center">🔑 Contributor Login</h3><form method="POST" autocomplete="on"><div class="mb-3"><input type="text" name="username" class="form-control" placeholder="Username" autocomplete="username" required></div><div class="mb-3"><input type="password" name="password" class="form-control" placeholder="Password" autocomplete="current-password" required></div><div class="mb-3 form-check"><input type="checkbox" name="remember" id="remember" class="form-check-input"><label class="form-check-label" for="remember">Remember me</label></div><input type="hidden" name="event_token" value="{{ request.args.get('event_token', '') }}"><button type="submit" class="btn btn-gold w-100">Login</button></form><div class="mt-3 text-center"><small><a href="{{ url_for('contributor_register', event_token=request.args.get('event_token', '')) }}">Create an account</a></small></div></div></div></div>
{% endblock %}
"""
...
def index():
    if is_admin_logged_in():
        ...
        if admin is None:
            session.clear()
            flash('Your session was invalid. Please login again.', 'error')
